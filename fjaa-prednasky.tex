...
Je to způsobeno jinou nejednoznačností než tou, ktrerou jsme eliminovali pomocí nejlevější derivace.
\end{priklad}
...
\begin{veta}
Pokud $X \Rightarrow_{G,l}^* x$ pak exstuje derivační strom $x$ z $X$ podle $G$.
\end{veta}
\begin{dukaz}
...
\subsection{Jednoznačené a nejednoznačné bezkontextové gramatiky}
...
\begin{dukaz}
Pokud $L$ je regulární, pak existuje KDA $A$ s úplnou přechodovu funkcí $\delta$. Pak pro $A$ lze sestavit gramatiku $G$ tak, že bude platit $L(A) = L(G)$.
