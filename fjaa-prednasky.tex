...
Gramatika říká, v jakém tvaru může být zapsán validní program, tedy definuje všechna slova\footnote{Pojem \emph{slovo}
...
Například $\langle a_{1}, a_{2}, \ldots, a_{n} \rangle \in \Sigma, n$ nazvěme \textit{délkou řetězce}.
Formálně definujme řetězec jakožto \textit{zobrazení}
$$
...
\item
\textit{Prázdný řetězec} - edná se o řetězec, pro který platí, že $|x| = 0$ a značíme jej $\varepsilon$, přičemž platí následující zápis:
$$
...
\begin{itemize}
\item
\textit{Prefix} řetězce. Označme jej $Pfx(x) = \lbrace y | \exists z \text{ tak, že } yz = x \rbrace$.

\item
\textit{Infix} řetězce. Označme jej $Ifx(x) = \lbrace y | \exists z_{1}, z_{2} \text{ tak, že } z_{1}yz_{2} = x \rbrace$.

\item
\textit{Sufix} řetězce. Označme jej $Sfx(x) = \lbrace y | \exists z \text{ tak, že } zy = x \rbrace$.
\end{itemize}
