...
\renewcommand{\lstlistingname}{Algoritmus}			% Listing -> Algoritmus
...
\abstract{
Tento dokument je pouze přepisem zápisků a poznámek z přednášek předmětu KMI/FJAA. Přednášel \link{doc. Vilém Vychodil Ph.D.}{http://vychodil.inf.upol.cz/}. Do tohoto dokumentu přispěli opravami i další lidé, jmenovitě A.~Baron}
...
\section{Historie}
Počátek úvah, jež byly později základem seriozního zkoumání formálních jazyků potažmo automatů se datuje do 30. let 20. století.
Průkopníkem této oblasti byl \link{Noam Chomsky}{http://cs.wikipedia.org/wiki/Noam_Chomsky}
\footnote{Jméno této osoby čti [\textit{čomski}] a zapamatuj si ke státnicím, že Chomsky byl \textit{nebezpečný levicový intelektuál}.}.

Důsledná (až antropologická) úvaha nad tvarem jazyků (opravdu nemáme na mysli vepřovy jazyk) je základním kamenem k efektivní a
přívětivé implementaci překladačů, rozhodovacích systémů nebo programovacích jazyků obecně. Je proto nanejvýš vhodné proniknout do tajů
vytváření jazyků (generativní formalismus) a jejich rozpoznávání a analýzy (analytický formalismus).

Jako příklad selhání autora programovacího jazyka si uveďme jazyk \textit{Fortran}, jehož konstrukce byla po syntaktické stránce špatná,
což vedlo ke \textit{gramatické ne jednoznačnosti} tohoto jazyka.

\section{Kódová analýza}
Analýza kódu je elementárním stavebním kamenem analýzy jazyka, jenž je tímto kódem reprezentován, protože
existuje (ne nutně silná) spojitost mezi syntaktickou \uv{krásou} kódu a následnou kvalitou (případně sémantikou) daného jazyka atp.


\subsection{Lexikální analýza}
V této analýze hraje prim dělení vstupního kódu na tokeny\footnote{Překládej jako \textit{část, díl nebo také fráze}.},
jež se zapisují ve stylu:
$$\langle\text{ \textcolor{red}{znak}, \textcolor{blue}{identifikátor} }\rangle$$
Příkladem je tedy i token $\langle\text{ \textcolor{red}{=}, \textcolor{blue}{assignment} }\rangle$, případně další tokeny.

\subsection{Syntaktická analýza}
Syntaktická analýza vytváří stromovou závislost jednotlivých tokenů, jejíž reprezentace se nazývá \textit{derivační strom}.
V rámci této analýzy rozlišme:
\begin{enumerate}
\item
Teorii jazyků, jež se zabývá stavbou jazyka (respektive jeho syntaxí) a poskytuje tzv. \textit{generativní aparát}.
Gramatika říká, v jakém tvaru může být zapsán validní program, tedy definuje všechna slova\footnote{Pojem \textit{slovo}
je v tomto kontextu poněkud zavádějící. V některé z následujících kapitol bude zaveden přesnější termín na vystihnutí prvku jazyka},
která jsou obsažena v daném jazyce.

\item
Teorii automatů, jež poskytuje tzv. \textit{analytický aparát}.
Automatem se rozumí v podstatě jednoduchý algoritmus, který rozhoduje zdané dané vstupní slovo patří či nepatří do nějakého určitého jazyka.
\end{enumerate}

\section{Základní pojmy}
\begin{itemize}
\item
\textit{Symbol} (případně znak) - jedná se o syntaktický pojem (význam tedy nehraje roli), který představuje \textit{jméno} (analogicky k \textit{písmenu} z přirozeného jazyka).
Mezi symboly počítejme například \textit{0, +, Š, while}. Symbol může mít jakoukoliv zástupnou grafickou reprezentaci.

\item
\textit{Abeceda} - abecedou rozumíme množinu (například množinu $X$) všech přípustných \textit{symbolů} (znaků), přičemž taková množina je neprázdná (tedy $|x|>0$) a konečná.
...
Například $\Sigma = \lbrace a, b, c \rbrace$, tedy \uv{abeceda $\Sigma$ obsahuje symboly \textcolor{red}{a, b, c}.}

\item
\textit{Řetězec} - jedná se o konečnou posloupnost symbolů (znaků) vybraných z nějaké dané abecedy.
Například \textit{n}-tice $\langle a_{1}, a_{2}, \ldots, a_{n} \rangle \in \Sigma$ je řetězecem.

Formálně definujme řetězec jakožto \textit{zobrazení} ve tvaru:
$$
...
$$ Vidíme, že se jedná o zobrazení z přirozených čísel do symbolů z nějaké abecedy, v tomto případě do abecedy $\Sigma$.
Délku řetězce označme $|x|$.

\item
\textit{Prázdný řetězec} - jedná se o řetězec, pro který platí, že $|x| = 0$ a značíme jej $\varepsilon$, přičemž platí, že:
$$
\varepsilon \subseteq \emptyset \rightarrow \Sigma
$$
Prázdný řetězec neobsahuje byť jeden symbol, tedy $\varepsilon \notin \Sigma$. Jedná se o jakýsi imaginární prvek,
jehož role je však klíčové, jak se dozvíme později.
\end{itemize}
...
\begin{poznamka}
Předchozí věta je relativně intuitivní, protože pokud máme $k$-prvkovou abecedu a tvoříme z ní řetězec,
pak na každé místo v tomto řetězci můžeme vybrat právě jeden z $k$ symbolů abecedy. Avšak takto volit bude hned $n$-krát,
protože konstruujeme řetězec délky $n$. Tato úvaha vede na kombinatorický problém počtu variací s opakováním, který
se dá spočíst dle notoricky známeho vzorce $V(n,k) = k^n$.
\end{poznamka}

Uveďme si značení pro dva důležité pojmy:
\begin{itemize}
\item
...
\begin{itemize}
\item
\textit{Zřetězení} (konkatenace) - jde v podstatě o spojení\footnote{Pro milovníky jazyka Scheme můžeme tuto operaci přirovnat k proceduře \textit{append}}
dvou řetězců v daném pořadí do jednoho řetězce.

\begin{priklad}
Mějme dva řetězce $a = a_{1}\ldots a_{n}$, $b = b_{1}\ldots b_{m}$, pak jejich zřetězení má tvar:
$$
a_{1}\ldots a_{n}b_{1}\ldots b_{m}
$$
Identifikátorem\footnote{Identifikátor zřetězení se velmi často v zápisech zřetězení vynechává. Je to analogická situace k násobení výrazů.} operace
zřetězení je \textcolor{red}{$\circ$}, například $x\circ y$ je zřetězením řetězců $x$ a $y$.
Formálně takto:
\begin{gather*}
...
\item
\textit{Rovnost řetězců} - pro prohlášení dvou řetězců za sobě rovné v žádaném smyslu je třeba splnit obecně dvě následující podmínky:
\begin{enumerate}
\item
Oba řetězce mají stejnou délku, tedy $|x| = |y|$.

\item
Bude-li délka označena jako $n$, pak musí platit, že $\forall i |\quad i \in \lbrace 1, \ldots, n \rbrace, x(i) = y(i)$.
Tedy každé dva k sobě náležící symboly z daných řetězců jsou si rovny.
\end{enumerate}

Uvažujeme-li rovnost řetězců, pak je záhodno uvažovat:
\begin{itemize}
\item
\textit{Prefix} řetězce - značme jej $Pfx(x) = \lbrace y | \exists z \text{ tak, že } yz = x \rbrace$.

\item
\textit{Infix} řetězce - značme jej $Ifx(x) = \lbrace y | \exists z_{1}, z_{2} \text{ tak, že } z_{1}yz_{2} = x \rbrace$.

\item
\textit{Sufix} řetězce - značme jej $Sfx(x) = \lbrace y | \exists z \text{ tak, že } zy = x \rbrace$.
\end{itemize}

\begin{veta} Nechť platí:
\begin{gather*}
xy = xz \: \Longrightarrow \: y = z \\
yx = zx \: \Longrightarrow \: y = z
\end{gather*}
Nebo-li pokud máme k dispozici dva řetězece a ty jsou si rovny a navíc mají identický prefix, pak je jasné, že \uv{zadnice} těchto
řetězců mají stejnou délku a jsou si také rovny.
...
\item
\textit{N-tá mocnina} řetězce - je popsána následující definicí:
$$
x^{n} = \left\{
\begin{array}{l l}
...
tehdy, když $\exists z|\quad z \in \Sigma^{*} \text{ a nezáporná celá čísla } p, q \text{ tak, že } u = z^{p} \text{ a } v=z^{q}$.
\end{veta}

Předpokládejme, že pro $p, z, q$ máme $u = z^{p}, v = z^{q}$. Pak obecně platí, že:
$$
...
Navíc berme v potaz také následující zápis doplněný obrázkem \ref{obr-1}:
\begin{gather*}
uw = v \\
wu = v \\
uw = wu \\
...
Formálním jazykem obecně myslíme nějakou množinu \textit{řetězců}, které navíc většinou mají určitou společnou vlastnost,
jak uvidíme později.
	
\section{Lexikografické uspořádání}
Předpokládejme uspořádání na množině všech řetězců $\Sigma^{*}$.
Nazvěme toto uspořádání \textit{striktním totálním}. Pak toto uspořádání například pro $\Sigma = \lbrace a_{1}, \ldots, a_{n} \rbrace$
...
\item
$|x| = |y| \text{ a } \exists i \text{ tak, že } x(i) < y(i) \text{ a zároveň } x(j) = y(j) \text{ pro } \forall j |\quad j < i$
\end{enumerate}

\begin{priklad}
$\Sigma = \lbrace 0, 1 \rbrace$. Triviálně tedy máme $0 < 1$.
...
\item
\textit{Kleeneho\footnote{\link{Stephen Cole Kleene}{http://en.wikipedia.org/wiki/Stephen_Cole_Kleene} je známý matematik, jenž se významně podílel na položení základů teoretických počítačových věd.}} uzávěr neboli \textit{iterace}. Tento uzávěr vyjádříme takto:
$$
...
$P$ je množinou odvozovacích pravidel a $S \in N$\hyplabel{terminal} je tzv. \textit{počátečním} resp. \textit{startovním} neterminálem. Pak tuto čtveřici nazveme \textit{gramatikou}.
\end{definice}
...
Mějme tedy nějaké dané $\Sigma \text{ a } S$ a zvolme \textit{N}. Následně platí $\forall x_{i} \in L$ je dvojího typu:
...
\end{veta}
\begin{dukaz}
Mějme abecedu $\Gamma = \lbrace , , \rightarrow , \lbrace , \rbrace , a , n, \# \rbrace$.\footnote{Všechny prvky množiny jsou symboly, včetně symbolu \uv{čárka}} Pomocí $\Gamma$ jsme schopni kódovat jazyk pravidel gramatiky. (Neterminály můžeme kódovat jako $n, nn, \dots$).
...
Za \textit{determinismus} považujme takovou konfiguraci, pro kterou platí, že je v \textit{každém jejím kroku jasné}, co bude následovat.
Naopak u \textit{nedeterministických} konfigurací \textit{není v určitých případech možné další krok přesně vyjádřit} na základě znalostí aktuálního kroku.
...
Popišme si nyní jeden krok výpočtu, tj. obecně:
$$
...
\begin{poznamka}
Automat $A$ přijímá řetězec $a$, pokud $\delta^*(q_0,w) \in F$.
\end{poznamka}
...
\vspace{10px}
\begin{lstlisting}[caption=Pseudokód pro převod KNA na KDA.,mathescape]
$\delta^{D}\leftarrow\emptyset$; $Q^{D}\leftarrow\emptyset$; $F^{D}\leftarrow\emptyset$; $W\leftarrow{I}$
...
while $W\neq{Q}$ do
     ...
     foreach $a\in\Sigma$ do
         ...
         if $N\neq\emptyset$ then
             ...
             $\delta^{D}\leftarrow\delta^{D}\cup{<R, a, N>}$
         endif
     end
end
...
\begin{definice}
\textit{Trie} je prefixový strom, který umožňuje \uv{rychlé hledání ve slovníku.}
\end{definice}

\begin{definice}
Jako \textit{slovník} označujeme konečný neprázdný jazyk který neobsahuje $\varepsilon$.
\end{definice}
...
Nechť $A=\langle\Sigma,Q,\delta,I,F\rangle$ je \ekna. Řetězec $w \in \Sigma^*$ nazýváme řetězec \textit{přijímaný} $A$, pokud
...
Máme $A = \langle \Sigma, Q, \delta, I, F \rangle$ a víme, že \textit{W} je neprázdná množina dosud nezpracovaných stavů.

\begin{lstlisting}[caption=Pseudokód pro převod $\varepsilon$ KNA na KDA.,mathescape,float]
$\delta^{s}\leftarrow\emptyset$; $Q^{s}\leftarrow\emptyset$; $F^{s}\leftarrow\emptyset$; $w\leftarrow{E}_{A}(I)$
...
\EdgeR{A1}{B0}{\varepsilon}
\EdgeR{A2}{B0}{\varepsilon}
\end{VCPicture}
\end{center}
...
\begin{VCPicture}{(-6,-4)(6,4)}
\StateVar[{r_0}]{(-6,0)}{r0}
\StateVar[{r_i = r_j}]{(0,0)}{rirj}
\FinalStateVar[{r_m}]{(6,0)}{rm}
...
\begin{eqnarray*}
\Gamma &-& \text{ abeceda zásobníkových symbolů.} \\
...
\begin{itemize}
\item
Prvnímu případu říkáme \textit{operace srovnání}: pokud je na vstupu i na vrcholu zásobníku stejný znak (terminál) pak jsou oba odebrány.
\item
Druhý případ se jmenuje \textit{operace expanze}: bez čtení vstupu při neterminálu na vrcholu zásobníku se tento neterminál nahradí pravou stranou některého pravidla s tímto neterminálem na levé straně.
\item
...
\begin{dukaz}
Nechť platí 1. tvrzení: $L=N(A)$ \textit{pro DZA A}

\begin{enumerate}
\item
pozorování: $L$ má prefixovou vlastnost

$x \in L=N(A)$ tj. $x$ je přijat vyprázdněním zásobníku. Zjišťujeme, že $\langle q_0,x,z_0 \rangle \vdash^* \langle p,\varepsilon,\varepsilon \rangle$ je \textit{jednoznačně} daný výpočet.
