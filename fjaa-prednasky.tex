...
\renewcommand{\listingname}{Algoritmus}			% Listing -> Algoritmus
...
\begin{listing}
\begin{alltt}
\(\delta\sp{D}\leftarrow\emptyset; Q\sp{D}\leftarrow\emptyset; F\sp{D}\leftarrow\emptyset; w\leftarrow{I} \)

while \(w\neq{Q}\) do
     select \(R\in{w}\)
     \(w\leftarrow{w}\smallsetminus{R}; Q\sp{D}\leftarrow{Q}\sp{D}\cup{R}\)
     if \(R\cap{F}\neq\emptyset\) then
         \(F\sp{D}\leftarrow{F}\sp{D}\cup{R}\)
     endif
     foreach \(a\in\Sigma\) do
         \(N\leftarrow\delta\sp{*}(R, a)\)
         if \(N\neq\emptyset\) then
             if \(N\notin{w}\cup{Q}\sp{D}\) then
                 \(w\leftarrow{w}\cup{N}\)
             endif
             \(\delta\sp{D}\leftarrow\delta\sp{D}\cup{<R, a, N>}\)
         endif
     end
end
return \(<\Sigma, Q\sp{D}, \delta\sp{D}, I, F\sp{D}>\)
\end{alltt}
